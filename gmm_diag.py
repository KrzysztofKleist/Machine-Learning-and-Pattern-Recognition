...
if __name__ == '__main__':
    ...
    for components in componentsRange:
        ...
        classifierType = "GMM Diag "
        for k in kRange:
            print("k: {}".format(k), "\r", end="")
            for iter in range(k):
                DTrain, DTest, LTrain, LTest = kFolds(D, L, k, iter)
                log_scores, labels = compute_gmm_diag_matrix(DTrain, LTrain, DTest, LTest, labels, log_scores,
                                                             components,
                                                             iterations)
            ...
            plt.savefig('plots/roc_plot_gmm_diag_cov_raw_data_np_pca_components_2.jpg')
            # plt.savefig('plots/roc_plot_gmm_diag_cov_gaussianized_data_np_pca_components_4.jpg')
            plt.show()

            # Bayes error plot
            bayes_error_plot(np.linspace(-1, 1, 21), scores, labels)
            plt.savefig('plots/bayes_error_plot_gmm_diag_cov_raw_data_np_pca_components_2.jpg')
            # plt.savefig('plots/bayes_error_plot_gmm_diag_cov_gaussianized_data_np_pca_components_4.jpg')
            plt.show()
